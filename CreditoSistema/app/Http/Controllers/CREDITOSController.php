...
{ 
 ...
 public function Insertar(Request $request){
    ...
    $interes_orig = DB::table('dbo.tbl_creditos_plan_pagos')
    ->where('dbo.tbl_creditos_plan_pagos.no_credito', $no_credito)
    ->whereNotNull('interes_orig')
    ->take(1)
    ->value('dbo.tbl_creditos_plan_pagos.interes_orig');

    //return  $query;
    
    $query1= DB::select('select max(fecha_cuota) as fecha_cuota from tbl_creditos_plan_pagos where no_Credito = ?', [$no_credito]);

    $query8= DB::select('select max(no_cuota) as no_cuota from tbl_creditos_plan_pagos where no_Credito = ?', [$no_credito]);
   // $fecha= DB::select('select fecha_cuota from tbl_creditos_plan_pagos where no_Credito = 3916 ');
   


   return view('InsertarPrestamoVen', ['Creditos'=> $query], ['count'=>$query1, 'num' => $query8, 'interes_or' => $interes_orig]);
 // return $planPago;
    ...
    public function filtros(Request $request){
        ...
        'tbl_creditos_plan_pagos.capital_orig',
        'tbl_creditos_plan_pagos.MORA',
        'tbl_creditos_plan_pagos.StatusCuota',
        'tbl_creditos_plan_pagos.ajuste_fecha',
        'TBL_CLIENTES.tasa_mora',
        'tbl_creditos_plan_pagos.comision',
        'tbl_creditos_plan_pagos.fecha_cuota',
       ...
       /* foreach($query2 as $l){
         ...
         //Alert::warning('Verificar Frecuencia', 'warning');
         toast('¡Verificar Frecuencia!','warning');
         
            return view('clientesfiltros', ['Creditos'=> $query], ['count'=>$query1]);
  ...
  /*$query=DB::table('dbo.tbl_creditos_plan_pagos')
    ...
    public function UpdatePlanPago(Request $request){
            ...
            $moraperiodo  = ($request->get('moraperiodo'));
            $no_cuota     = ($request->get('no_cuota'));
            $buscarpor    = ($request->get('buscarpor'));
            $ajuste_fecha = ($request->get('ajuste_fecha'));
            $StatusCuota  = ($request->get('StatusCuota'));
            $INT          = ($request->get('INT'));
            $interes      = ($request->get('interes'));

        for ($i= 0; $i < count($moraperiodo); $i++) {
                $DataUpdate = [
                'cuota' =>$cuota[$i],
                'mora' => $moraperiodo[$i],
                'no_cuota' => $no_cuota[$i],
                'no_credito' =>  $buscarpor[$i],
                'ajuste_fecha' =>   $ajuste_fecha[$i],
                'StatusCuota' =>  $StatusCuota[$i],
                'otros' =>           $INT[$i],
                'interes' =>  $interes[$i],
       ...
       ->where('tbl_creditos_plan_pagos.no_cuota', $no_cuota[$i])
        ...
        ->where('tbl_creditos_plan_pagos.fecha_cuota', '<=' , today())
        ->where('tbl_creditos_plan_pagos.estado', '=' ,'p')
        ->update(array('tbl_creditos_plan_pagos.MORA' =>  $moraperiodo[$i]));
        ...
        ->where('tbl_creditos_plan_pagos.no_cuota', $no_cuota[$i])
         ...
         ->where('tbl_creditos_plan_pagos.fecha_cuota', '<=' , today())
         ->where('tbl_creditos_plan_pagos.estado', '=' ,'p')
         ->update(array('tbl_creditos_plan_pagos.ajuste_fecha' =>  $ajuste_fecha[$i]));
         ...
         ->where('tbl_creditos_plan_pagos.no_cuota', $no_cuota[$i])
          ...
          ->where('tbl_creditos_plan_pagos.fecha_cuota', '<=' , today())
          ->where('tbl_creditos_plan_pagos.estado', '=' ,'p')
          ->update(array('tbl_creditos_plan_pagos.StatusCuota' =>  $StatusCuota[$i]));
        ...
        ->where('tbl_creditos_plan_pagos.no_cuota', $no_cuota[$i])
         ...
         ->where('tbl_creditos_plan_pagos.fecha_cuota', '<=' , today())
         ->where('tbl_creditos_plan_pagos.estado', '=' ,'p')
         ->update(array('tbl_creditos_plan_pagos.otros' =>  $INT[$i]));
        ...
        ->where('tbl_creditos_plan_pagos.no_cuota', $no_cuota[$i])
         ...
         ->where('tbl_creditos_plan_pagos.fecha_cuota', '<=' , today())
         ->where('tbl_creditos_plan_pagos.estado', '=' ,'p')
         ->update(array('tbl_creditos_plan_pagos.comision' => $interes[$i]));
         ...
         Alert::success('Plan Pago agregado con exito', 'success');

          // dd($request->all());
          //return redire$paginator->nextPageUrl();
    ...
    public function ActualizarC(Request $request){
        ...
        $query1 = DB::table('dbo.tbl_creditos_plan_pagos')
         ...
         ->update(array('tbl_creditos_plan_pagos.comision_orig' =>  $comision_orig));
    ...
    public function InsertarCuota(Request $request){
        ...
        DB::insert('insert into tbl_creditos_plan_pagos (no_credito, no_cuota, cuota, capital, interes, saldo, fecha_cuota, comision, mora, estado, interes_orig, ajuste_fecha, otros ) values (?,?,?,?,?,?,?,?,?,?,?,?,?)', [$no_credito[$i], $no_cuota[$i],$cuota[$i] + $mora[$i], 0.00,$interes[$i],0.00,$fecha_cuota[$i],$comision[$i] + $otros[$i],$mora[$i],'P', $interes_orig, $ajuste_fecha[$i],$otros[$i]]);
        }
        //dd($interes_orig);
        return  Redirect()->back();
    } 

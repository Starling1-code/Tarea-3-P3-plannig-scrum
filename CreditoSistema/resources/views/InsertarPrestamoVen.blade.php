...
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.2.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-Zenh87qX5JnK2Jl0vWa8Ck2rdkQ2Bzep5IDxbcnCeuOxjzrPF/et3URy9Bv1WTRi" crossorigin="anonymous">
  ...
  @foreach($Creditos as $l)
        ...
        /*  $cuota = ($l->capital_orig + $l->interes_orig + $l->comision_orig + $l->otros);
          ...
          // $date_future = strtotime($l->fecha_ultimo_pago, '+15 day');
      ...
      // $Mora_periodos = $Mora * $IntentosF2;
         ...
         $CuotaInsertar = ($l->capital + $l->INTERES + $l->comision + $Mora_periodos );

        }elseif($l->cod_frecuencia_pago_Cuota == 4){

          $IntentosF = $Dias_Venc /  15;
          ...
          $IntentosF = ($Dias_Venc /  $dias_meses_Frec2) + 1;
          $IntentosF2 = floor($IntentosF);
       ...
       // $sumador_mora    = $sumador_mora    + $l->MORA;
        ...
        $sumador_capital = $sumador_capital + $l->capital;
          ...
          @endphp
            ...
            <td>{{number_format($CuotaInsertar, 2)}}</td>
            <td>{{$l->capital}}</td>
            <td>{{$l->INTERES}}</td>
            <td>{{$l->comision}}</td>
  ...
  @endforeach
      ...
      @foreach ($count as $ls)
          @php
              $datos1 = calcularTiempo($ls->fecha_cuota, $FechaFinal);
              $años1 = $datos1[0];
              $meses1 = $datos1[1];
              $dias1 = $datos1[2];

              $m_p_v1 = $años1 * $mesesA + $meses1;//De años a meses
              $Dias_Venc1 = ($m_p_v1 * $dias_meses); //De meses a dias
              $Dias_Venc1 = $Dias_Venc1 + $dias1;
              ...
              if($l->cod_frecuencia_pago_Cuota == 5){
                ...
                $IntentosF = $Dias_Venc1 /  $dias_meses_Frec5;
              $IntentosF3 = number_format($IntentosF, 2, '.', '');
  
              }elseif($l->cod_frecuencia_pago_Cuota == 4){

                $IntentosF = $Dias_Venc1 /  $dias_meses_Frec4;
              $IntentosF3 = number_format($IntentosF, 2, '.', '');


              }elseif($l->cod_frecuencia_pago_Cuota == 3){

                $IntentosF = $Dias_Venc1 /  $dias_meses_Frec3;
              $IntentosF3 = number_format($IntentosF, 2, '.', '');


              }elseif($l->cod_frecuencia_pago_Cuota == 2){

                $IntentosF = $Dias_Venc1 /  $dias_meses_Frec2;
              $IntentosF3 = number_format($IntentosF, 2, '.', '');
        ...
        @endforeach
              ...
              $fechaInicial = date($ls->fecha_cuota);
              ...
              $fechaFinal = date('Y-m-d h:i:s' , $fechaEnSegundos);
                 ...
                 @endphp
                  ...
                  <form action="{{route('credito.InsertarCuota')}}" method="get">
          
          @for ($i=0; $i < $IntentosF3; $i++)
          ...
          /*  $cuota = ($l->capital_orig + $l->interes_orig + $l->comision_orig + $l->otros);
            ...
            //$INTENTOS = ($INT_F * $contador - 100);



              $cuotaSumar = floor($num[0]->no_cuota + $contador);
              ...
              $dias_meses_Frec51 = $dias_meses_Frec51 + 30;

              $fecha1 = date("Y-m-d",strtotime($FechaCuota."+".$dias_meses_Frec51. "days")); 

              $datos1 = calcularTiempo($fecha1, $FechaFinal);
                $años2 = $datos1[0];
                $meses2 = $datos1[1];
                $dias2 = $datos1[2];

                $m_p_v2 = $años2 * $mesesA + $meses2;//De años a meses
                $Dias_Venc2 = ($m_p_v2 * $dias_meses); //De meses a dias
                $Dias_Venc2 = $Dias_Venc2 + $dias2;

            $IntF4 = $Dias_Venc2 /  $dias_meses_Frec5;
            $IntF5 = number_format($IntF4, 2, '.', '');


            $INT1 = ($INT_F * $IntF5);

            $cuota = ( $l->interes_orig + $l->comision_orig + $INT1);
              $TasaCuota = 10;
              $Porcentaje = 100;
              $Mora = ($cuota) * $TasaCuota / $Porcentaje;
                ...
                $Mora_final = $Mora * ($Dias_Venc2 / 30);
                $comision = ($l->comision_orig);
                $Interesc = floor($l->interes_orig);
          ...
          //$INTENTOS = ($INT_F * $contador - 100);
            ...
            $dias_meses_Frec41 = $dias_meses_Frec41 + 15;

            $fecha1 = date("Y-m-d",strtotime($FechaCuota."+".$dias_meses_Frec41. "days")); 
          ...
          $IntF4 = $Dias_Venc2 /  $dias_meses_Frec4;
          $IntF5 = number_format($IntF4, 2, '.', '');
          ...
          $cuota = ( $l->interes_orig + $l->comision_orig + $INT1);
            ...
            //$INTENTOS = ($INT_F * $contador - 100);
              ...
              $dias_meses_Frec61 = $dias_meses_Frec61 + 14;

              $fecha1 = date("Y-m-d",strtotime($FechaCuota."+".$dias_meses_Frec61. "days")); 
            ...
            //$INTENTOS = ($INT_F * $contador - 100);
              ...
              $dias_meses_Frec2 = $dias_meses_Frec2 + 7;

              $fecha1 = date("Y-m-d",strtotime($FechaCuota."+".$dias_meses_Frec2. "days")); 
            ...
            $IntF4 = $Dias_Venc2 / $dias_meses_Frec2;
            $IntF5 = number_format($IntF4, 2, '.', '');
            ...
            $sumador_mora = $sumador_mora + $Mora_final;
            $sumador_seguro = $sumador_seguro + $comision;
            $sumador_inten =  $sumador_inten + $INT1;
            $sumador_interes = $sumador_interes + $l->interes_orig;
            ...
            @endphp
              ...
              <td></td>
              <td></td>
              <td><input class="editint" type="number" name="no_credito[]"  value="{{$l->no_credito}}"></td>
              <td></td>
              <td></td>
              <td></td>
              <td><input class="editint" type="number" name="no_cuota[]"  value="{{ $cuotaSumar }}"></td>
              <td></td>
              <td></td>
              <td></td>
              <td></td>
              <td></td>
              <td></td>
              <td><input class="editint" type="number" name="cuota[]"  value="{{$cuota = number_format( $cuota, 2, '.', '')}}"></td>
              <td></td>
              <td><input class="editint" type="number" name="interes[]"  value="{{ $interes_orig = number_format( $interes_or, 2, '.', '')}}"></td>
              <td><input class="editint" type="number" name="comision[]"  value="{{ $comision = number_format( $comision, 2, '.', '')}}"></td>
              <td><input class="editint" type="number" name="mora[]"  value="{{ $Mora_final = number_format( $Mora_final, 2, '.', '')}}"></td>
              <td><input class="editint"  name="fecha_cuota[]"  value="{{$fecha1}}"></td>
              <td>{{ $Dias_Venc2 }}</td>
              <td>{{$IntF5}}</td>
              <td><input class="editint" type="number" name="otros[]"  value="{{ $INT1 = number_format( $INT1, 2, '.', '')}}"></td>
              <td><input class="editint"  name="ajuste_fecha[]"  value="{{$FechaActual}}"></td>
              </tr>
              ...
              @endfor
              <button type="submit" class="CoOrig1">Insertar</button>
            </form>
    ...
    <div class="table_fotter d-flex justify-content-center">       
      ...
      <button type="submit" class="CoOrig2">Actualizar</button>
    </form>
      @endif
      </div>
      <p class="total">Total Pendientes : </p>
      </div>
  </div>
  <section class="modal ">
